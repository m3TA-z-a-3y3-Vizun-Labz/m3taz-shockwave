...
- Shockwave user settings persist at `~/Library/Application Support/shockwave/settings.json` (`codingAgent`, workspaces, `agentSecrets`, `tts`); active workspace is the iCloud Drive root — large scope slows the pi agent. ElevenLabs TTS: Settings → TTS, chat speak-aloud; key via safeStorage; IPC `tts:speak` / `tts:listVoices`.
...
- GitHub fork is `m3TA-z-a-3y3-Vizun-Labz/m3taz-shockwave` (renamed from `EagleEyeVisionLabz`; origin URL may still show the old name). [PR #1](https://github.com/m3TA-z-a-3y3-Vizun-Labz/m3taz-shockwave/pull/1) on `cursor/github-fetch-and-electron-install` (mainFetch, ensure-electron, GLM-5.2, Dify bundle, ElevenLabs TTS). Upstream `stephengpope/shockwave` ships desktop builds (v1.0.8).
- Main-process outbound HTTP uses `src/main/mainFetch.ts` (Electron `net.fetch` with Node `fetch` fallback) for GitHub Sync verify/API, update checks, voice tokens, TTS, and agent connection validation.
...
- `.cursor/hooks/state/` (continual-learning index) is machine-local with absolute transcript paths — keep untracked / out of commits; add to `.gitignore` if not already ignored.
